...
export default function Hero({ onNavigateCalculator, theme }) {
  ...
  return (
    <section id="hero" className={`relative py-12 sm:py-20 lg:py-24 overflow-hidden transition-colors duration-300 ${
      isDark ? 'bg-slate-900 text-white' : 'bg-amber-50/30 text-slate-900'
    }`}>
      ...
      <div className={`absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] sm:w-[650px] h-[350px] sm:h-[650px] rounded-full blur-[140px] pointer-events-none ${
        isDark ? 'bg-amber-500/15' : 'bg-amber-400/25'
      }`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-center">
          ...
          >
            <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs sm:text-sm font-semibold ${
              isDark ? 'bg-amber-500/15 border-amber-400/40 text-amber-300' : 'bg-amber-100 border-amber-300 text-amber-800 shadow-xs'
            }`}>
            ...
            <p className={`text-sm sm:text-lg max-w-2xl mx-auto lg:mx-0 font-normal leading-relaxed ${
              ...
              Індивідуальне проектування, професійний монтаж та пусконалагодження СЕС <strong className={isDark ? 'text-white' : 'text-slate-900'}>від 5 кВт до 1 МВт</strong> від <strong className={isDark ? 'text-white' : 'text-slate-900'}>Чедрика Івана</strong>. Гібридні системи Deye, автономне живлення EcoFlow та безпечний електромонтаж.
            </p>
              ...
              <a
                ...
                className={`glass-card font-semibold text-sm sm:text-base px-6 py-3.5 sm:py-4 rounded-xl border transition-all flex items-center justify-center gap-2 ${
                  isDark 
                    ...
                    : 'text-slate-700 hover:text-amber-600 border-amber-200 hover:border-amber-400 shadow-xs'
                }`}
              >
            ...
            <div className={`pt-6 border-t grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 text-xs font-semibold ${
              isDark ? 'border-slate-800 text-slate-300' : 'border-amber-200/80 text-slate-600'
            }`}>
          ...
          >
            <div className={`glass-panel p-5 sm:p-8 rounded-3xl border shadow-2xl relative overflow-hidden ${
              ...
              <div className={`flex items-center justify-between pb-5 border-b ${
                isDark ? 'border-slate-700' : 'border-amber-100'
              }`}>
              ...
              <div className="py-5 space-y-4">
                <div className={`p-3.5 sm:p-4 rounded-xl border space-y-1.5 ${
                  isDark ? 'border-slate-700 bg-slate-900/90' : 'border-amber-100 bg-amber-50/50'
                }`}>
                  <div className="flex justify-between text-xs font-medium">
                    <span className={isDark ? 'text-slate-300' : 'text-slate-600'}>Ефективність інвертора</span>
                ...
                <div className={`p-3.5 sm:p-4 rounded-xl border space-y-1.5 ${
                  ...
                  <div className="flex justify-between text-xs font-medium">
                    <span className={isDark ? 'text-slate-300' : 'text-slate-600'}>Економія електроенергії</span>
              ...
              <div className={`p-3.5 rounded-2xl border border-amber-400/30 flex items-center gap-3 ${
                isDark ? 'bg-amber-500/15 text-slate-200' : 'bg-amber-50 text-amber-900'
              }`}>
            ...
            <div 
              ...
              className={`glass-card p-4 sm:p-6 rounded-2xl border transition-all group ${
                isDark 
                  ...
                  : 'border-amber-200/80 bg-white hover:border-amber-400 shadow-xs'
              }`}
            >
              ...
              <p className={`text-[11px] sm:text-xs mt-1 ${isDark ? 'text-slate-300' : 'text-slate-500'}`}>
                {item.desc}
              </p>
            </div>
          ))}
